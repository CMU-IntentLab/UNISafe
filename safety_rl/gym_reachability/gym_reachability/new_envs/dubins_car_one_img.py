...
class DubinsCarOneEnvImg(gym.Env):
  ...
  def step(self, action):
    ...
    if self.doneType == "toEnd":
      done = not self.car.check_within_bounds(self.state) # TODO: Check the ending rule.
    elif self.doneType == "fail":
      done = fail or (not self.car.check_within_bounds(self.state))
    else:
      raise ValueError("invalid done type!")
    
    if not self.car.check_within_bounds(self.state):
      g_x = 1. # Out of the boundaries: safe.
  ...
  def get_warmup_examples_offline(self, num_warmup_samples=100):
    assert self.car.use_wm
    heuristic_v = np.zeros((num_warmup_samples, self.action_space.n))

    states = np.zeros((num_warmup_samples, self.car.feat_size))
    for i in range(num_warmup_samples):
      post, state0 = self.car.sample_random_state()
      states[i] = self.car.wm.dynamics.get_feat(post).detach().cpu().numpy().squeeze()

    with torch.no_grad():
      g_x = self.car.MLP_margin(torch.tensor(states, dtype=torch.float32).cuda()).cpu().numpy().squeeze()
  ...
  def get_uncertainty_values(self, theta, nx=101, ny=101, nz=None):
    ...
    with torch.no_grad():
      ...
      #Penn
      disagreement = self.car.disagreement_ensemble._intrinsic_reward_penn(repeated_states, repeated_actions)
  ...
  def visualize(
      ...
      labels=None, boolPlot=False, addBias=False, theta=None,
  ...
  def get_gt_brt(self, orientation, path):
    grid = np.load(path)
    nx = np.shape(grid)[0]
    ny = np.shape(grid)[1]
    nz = np.shape(grid)[2]
    ...
    if diff > 0:
      ...
      return self.get_grid_value(grid)[:,:,idx]
    else:
        idx2 = idx+1
  ...
  def plot_grid_values(self, ax, orientation, path, fig=None, vmin=-1, vmax=1, cmap="seismic", plot=True):
  ...
  def plot_uncertainty_values(
    ...
    if theta is None:
      theta = 0.
    v = self.get_uncertainty_values(theta, nx, ny)
