"""
Reachability SAC Training with Failure Prediction

This script trains a SAC (Soft Actor-Critic) agent for reachability analysis using
a world model environment that incorporates both epistemic uncertainty quantification
and explicit failure prediction. The agent learns to navigate safely by receiving
rewards based on both model uncertainty and predicted failure probabilities.

Key Features:
- World model-based training with pre-trained dynamics
- Failure prediction integration for enhanced safety
...
class ReachabilityTrainerWithFailurePrediction:
	"""
	Reachability-aware SAC trainer that uses world models with failure prediction.
	
	This class orchestrates the training of a SAC agent in a world model environment
	that provides safety-oriented rewards based on both epistemic uncertainty and
	explicit failure predictions. The agent learns to avoid regions where the model
	is uncertain or failure is likely.
	...
	def __init__(self, config: Any):
		"""
		Initialize the reachability trainer with failure prediction capabilities.
	...
	def setup_world_model(self) -> None:
		...
		2. Failure prediction head for safety classification
		3. Disagreement ensemble for epistemic uncertainty quantification
		...
		print(f"  - Failure prediction head available: {'failure' in self.wm.heads}")
	...
	def initialize_agent(self) -> None:
		"""
		Initialize the SAC agent for reachability learning with failure prediction.
		
		Creates a SAC agent configured for the world model feature space and
		sets up the environment to use failure prediction rewards. The agent
		will learn to navigate safely by avoiding both uncertain regions and
		states with high failure probability.
		"""
		...
		# Set up the agent with world model environment (failure prediction version)
		environment_name = "Takeoff_WM_Failure-v0"
		self.agent.setup_env(
			self.config, 
			self.wm, 
			self.disag_ensemble, 
			self.expert_dataset, 
		...
		print(f"SAC agent initialized with environment: {environment_name}")
	...
	def train_agent(self) -> None:
		...
		that combine epistemic uncertainty avoidance and failure prediction.
		The training uses the world model environment for efficient and safe
		policy learning.
		"""
		print("\n=== Training Reachability SAC Agent with Failure Prediction ===")
	...
	def execute(self) -> None:
		"""
		Main execution pipeline for reachability training with failure prediction.
		...
		The trained agent will learn to navigate safely by avoiding both
		uncertain regions (high epistemic uncertainty) and dangerous states
...
if __name__ == "__main__":
	...
	# Initialize wandb
	wandb.init(project="Isaac_Reachability", name=expt_name)
	wandb.run.log_code("source/latent_safety/reachability", exclude_fn=custom_exclude_fn)
	wandb.config.update(final_config)

	# Create the reachability trainer
	trainer = ReachabilityTrainerWithFailurePrediction(final_config)
	trainer.execute()
