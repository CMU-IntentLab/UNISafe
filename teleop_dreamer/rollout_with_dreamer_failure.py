...
# from reachability_dreamer.policy.DDPG_Reachability import DDPG 
from reachability_dreamer.policy.DDPG_Reachability_Env import DDPG 
	...
	def run(self):
		...
		while True:
			with torch.inference_mode():
				...
				failure = failure_dist.mean
				failure_uncertainty = failure_dist.entropy()
				...
				print("UQ: {:.4f} | failure: {:.4f}, {:.4f}".format(uncertainty, failure.item(), failure_uncertainty.item()))
...
if __name__ == "__main__":
	...
	configs = yaml.load(Path('source/latent_safety/dreamerv3_torch/configs.yaml').read_text())
	name_list = ["defaults"]
