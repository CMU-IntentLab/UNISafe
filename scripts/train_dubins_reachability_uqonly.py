...
class RARLAgent:
    ...
    def initialize_environment(self):
        ...
        env_name = "dubins_car_img_uqonly-v1"
    ...
    def setup_world_model(self):
        ...
        self.env.car.set_wm(wm, self.config)

        if self.config.use_ensemble and (not self.config.use_unit_ensemble):
        ...
        if self.config.use_ensemble and self.config.use_unit_ensemble:
            disag_ensemble = expl.OneStepPredictorUnitVariance(self.config, wm)
            state_dict = {k[16:]: v for k, v in wm_checkpoint['agent_state_dict'].items() if '_disag_ensemble' in k}
            disag_ensemble.load_state_dict(state_dict)
            self.env.car.set_ensemble(disag_ensemble, self.config)

        if self.config.use_density_estimator:
            density_estimator = expl.DensityEstimator(self.config, wm)
            state_dict = {k[19:]: v for k, v in wm_checkpoint['agent_state_dict'].items() if '_density_estimator' in k}
            density_estimator.load_state_dict(state_dict)
            self.env.car.set_density_estimator(density_estimator, self.config)
    ...
    def initialize_agent(self):
        ...
        CONFIG = dqnConfig(
        DEVICE=self.device, ENV_NAME="dubins_car_img_uqonly-v1", SEED=self.config.randomSeed,
    ...
    def save_training_results(self, trainRecords, trainProgress):
        trainDict = {'trainRecords': trainRecords, 'trainProgress': trainProgress}
        filePath = os.path.join(self.outFolder, 'train')
        save_obj(trainDict, filePath)

    def plot_training_results(self, trainRecords):
        fig, axes = plt.subplots(1, 1, figsize=(8, 4))
        data = trainRecords
        ax = axes[0]
        ax.plot(data, 'b:')
        ax.set_xlabel('Iteration (x 1e5)', fontsize=18)
        ax.set_xticks(np.linspace(0, self.config.maxUpdates, 5))
        ax.set_xticklabels(np.linspace(0, self.config.maxUpdates, 5) / 1e5)
        ax.set_title('loss_critic', fontsize=18)
        ax.set_xlim(left=0, right=self.config.maxUpdates)

        fig.tight_layout()
        wandb.log({"train_loss_success": wandb.Image(fig)})
        plt.close()

    def evaluate_agent(self):
        nx, ny = 41, 41
        xs = np.linspace(self.env.bounds[0, 0], self.env.bounds[0, 1], nx)
        ys = np.linspace(self.env.bounds[1, 0], self.env.bounds[1, 1], ny)
        ...
        while not it.finished:
            ...
            traj, result, info = self.env.simulate_one_trajectory_post(
                self.agent.Q_network, T=250, state=state, toEnd=False, nominal_policy=None
            )
            resultMtx[idx] = result
            it.iternext()

        # TODO : log closed-loop performance.
        self.plot_evaluation_results(xs, ys, resultMtx)

    def plot_evaluation_results(self, xs, ys, resultMtx):
        # Calculate the success rate
        successRate = np.mean(resultMtx == 1)  # Assuming result == 1 indicates success
        print(f"Success Rate: {successRate:.3f}")

        # Plot the results
        plt.figure(figsize=(10, 8))
        plt.imshow(
            resultMtx.T, 
            extent=(self.env.bounds[0, 0], self.env.bounds[0, 1], 
                    self.env.bounds[1, 0], self.env.bounds[1, 1]),
            origin="lower",
            cmap="coolwarm",
            alpha=0.8
        )
        plt.colorbar(label="Result (-1: Fail, 1: Success)")
        plt.title(f"Evaluation Result\nSuccess Rate: {successRate:.3f}")
        plt.xlabel("X Position")
        plt.ylabel("Y Position")
        # Log results directly to WandB
        wandb.log({
            "evaluation_plot": wandb.Image(plt.gcf()),  # Log the current figure
        ...
        # Print a message confirming logging
        print(f"Results logged to WandB: Success Rate: {successRate:.3f}")
...
if __name__ == "__main__":
    ...
    wandb.init(project="dubins_reachability_final", name=config.expt_name)
