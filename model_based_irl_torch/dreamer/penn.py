...
from dreamer.ensemble.ensemble_linear import EnsembleLinear
from dreamer.ensemble.utility import JensenRenyiDivergence


class EnsembleStochasticLinear(torch.nn.Module):
    def __init__(self, in_features, out_features, hidden_features, ensemble_size=3, activation='relu', explore_var='legacy', norm=True, residual=True, dt=0.1):
...
class EnsembleStochasticLinearUnitVariance(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        dis = ensemble_var.mean(dim=1, keepdim=True)   # shape: [B, 1]
