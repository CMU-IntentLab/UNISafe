...
class Logger:
    def __init__(self, logdir, step):
        ...
        # Initialize WandB
        wandb.init(project="dubins_wm", config={"logdir": str(logdir)}, name=name)
...
def fill_expert_dataset_dubins(config, cache, predefined_path=None):

    dataset_path = config.dataset_path

    if predefined_path:
        print("Load dataset path: {}".format(predefined_path))
        dataset_path = predefined_path
    ...
    cprint(
        f"Loaded buffer with {num_fill_trajs} from {dataset_path}",
        color="magenta",
        attrs=["bold"],
    )
